...
text_name <- data.frame(value = paste0(paste0("M",1:25)," (",num_N,")"))
  
  

data_N <-  readRDS("~/projects/wangxinfeng/CML/result_fit_Normal.rds")
data_N <- as.matrix(data_N$original_data)


data_CML <-  readRDS("~/projects/wangxinfeng/CML/result_fit_CML.rds")
data_CML <- as.matrix(data_CML$original_data)

data_R <-  readRDS("~/projects/wangxinfeng/CML/result_fit_remission.rds")
data_R <- as.matrix(data_R$original_data)
...
result <- res_25#[[9]]$cluster_number
best.k <- 25
